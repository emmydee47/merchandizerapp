...
class cstores_edit extends cstores {
	...
	function LoadFormValues() {
		...
		if (!$this->store_name->FldIsDetailKey) {
			$this->store_name->setFormValue($objForm->GetValue("x_store_name"));
		}
	...
	function RenderRow() {
		...
		$this->address->ViewCustomAttributes = "";
			...
			$this->store_name->EditValue = ew_HtmlEncode($this->store_name->CurrentValue);
	...
	function ValidateForm() {
		...
		if (!$this->store_name->FldIsDetailKey && !is_null($this->store_name->FormValue) && $this->store_name->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->store_name->FldCaption(), $this->store_name->ReqErrMsg));
		}
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $stores->State->FldCaption(), $stores->State->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_store_name");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $stores->store_name->FldCaption(), $stores->store_name->ReqErrMsg)) ?>");
...
fstoresedit.Lists["x_State"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fstoresedit.Lists["x_State"].Options = <?php echo json_encode($stores_edit->State->Options()) ?>;
...
<?php if ($stores->sn->Visible) { // sn ?>
	<div id="r_sn" class="form-group">
		<label id="elh_stores_sn" class="<?php echo $stores_edit->LeftColumnClass ?>"><?php echo $stores->sn->FldCaption() ?></label>
		<div class="<?php echo $stores_edit->RightColumnClass ?>"><div<?php echo $stores->sn->CellAttributes() ?>>
<span id="el_stores_sn">
<span<?php echo $stores->sn->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $stores->sn->EditValue ?></p></span>
</span>
<input type="hidden" data-table="stores" data-field="x_sn" name="x_sn" id="x_sn" value="<?php echo ew_HtmlEncode($stores->sn->CurrentValue) ?>">
<?php echo $stores->sn->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($stores->State->Visible) { // State ?>
	<div id="r_State" class="form-group">
		<label id="elh_stores_State" for="x_State" class="<?php echo $stores_edit->LeftColumnClass ?>"><?php echo $stores->State->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $stores_edit->RightColumnClass ?>"><div<?php echo $stores->State->CellAttributes() ?>>
<span id="el_stores_State">
<select data-table="stores" data-field="x_State" data-value-separator="<?php echo $stores->State->DisplayValueSeparatorAttribute() ?>" id="x_State" name="x_State"<?php echo $stores->State->EditAttributes() ?>>
<?php echo $stores->State->SelectOptionListHtml("x_State") ?>
</select>
</span>
<?php echo $stores->State->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($stores->store_name->Visible) { // store_name ?>
	<div id="r_store_name" class="form-group">
		<label id="elh_stores_store_name" for="x_store_name" class="<?php echo $stores_edit->LeftColumnClass ?>"><?php echo $stores->store_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $stores_edit->RightColumnClass ?>"><div<?php echo $stores->store_name->CellAttributes() ?>>
<span id="el_stores_store_name">
<input type="text" data-table="stores" data-field="x_store_name" name="x_store_name" id="x_store_name" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($stores->store_name->getPlaceHolder()) ?>" value="<?php echo $stores->store_name->EditValue ?>"<?php echo $stores->store_name->EditAttributes() ?>>
</span>
<?php echo $stores->store_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<textarea data-table="stores" data-field="x_address" name="x_address" id="x_address" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($stores->address->getPlaceHolder()) ?>"<?php echo $stores->address->EditAttributes() ?>><?php echo $stores->address->EditValue ?></textarea>
</span>
