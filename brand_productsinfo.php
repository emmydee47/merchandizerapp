...
//
// Table class for brand_products
//
class cbrand_products extends cTable {
	var $sn;
	var $brand_id;
	var $product;
	...
	function __construct() {
		...
		$this->TableVar = 'brand_products';
		$this->TableName = 'brand_products';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`brand_products`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sn
		$this->sn = new cField('brand_products', 'brand_products', 'x_sn', 'sn', '`sn`', '`sn`', 3, -1, FALSE, '`sn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sn->Sortable = TRUE; // Allow sort
		$this->sn->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sn'] = &$this->sn;

		// brand_id
		$this->brand_id = new cField('brand_products', 'brand_products', 'x_brand_id', 'brand_id', '`brand_id`', '`brand_id`', 3, -1, FALSE, '`brand_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// product
		$this->product = new cField('brand_products', 'brand_products', 'x_product', 'product', '`product`', '`product`', 200, -1, FALSE, '`product`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`brand_products`";
	}
	...
	function GetModalCaption($pageName) {
		global $Language;
		if ($pageName == "brand_productsview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "brand_productsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "brand_productsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("brand_productsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("brand_productsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "brand_productsadd.php?" . $this->UrlParm($parm);
		else
			$url = "brand_productsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("brand_productsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("brand_productsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("brand_productsdelete.php", $this->UrlParm());
	}
	...
	// Common render codes
		...
		if (strval($this->brand_id->CurrentValue) <> "") {
			$sFilterWrk = "`sn`" . ew_SearchString("=", $this->brand_id->CurrentValue, EW_DATATYPE_NUMBER, "");
		$sSqlWrk = "SELECT `sn`, `brand_name` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `other_brands`";
		$sWhereWrk = "";
		$this->brand_id->LookupFilters = array();
