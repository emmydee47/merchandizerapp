...
//
// Table class for product_categories
//
class cproduct_categories extends cTable {
	var $sn;
	var $category;
	var $description;
	var $category_image;
	...
	function __construct() {
		...
		$this->TableVar = 'product_categories';
		$this->TableName = 'product_categories';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`product_categories`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sn
		$this->sn = new cField('product_categories', 'product_categories', 'x_sn', 'sn', '`sn`', '`sn`', 3, -1, FALSE, '`sn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sn->Sortable = TRUE; // Allow sort
		$this->sn->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sn'] = &$this->sn;

		// category
		$this->category = new cField('product_categories', 'product_categories', 'x_category', 'category', '`category`', '`category`', 200, -1, FALSE, '`category`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->category->Sortable = TRUE; // Allow sort
		$this->fields['category'] = &$this->category;

		// description
		$this->description = new cField('product_categories', 'product_categories', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// category_image
		$this->category_image = new cField('product_categories', 'product_categories', 'x_category_image', 'category_image', '`category_image`', '`category_image`', 200, -1, TRUE, '`category_image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->category_image->Sortable = TRUE; // Allow sort
		$this->category_image->UploadAllowedFileExt = "png, jpg, gif";
		$this->category_image->UploadMaxFileSize = 100000;
		$this->fields['category_image'] = &$this->category_image;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`product_categories`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "product_categorieslist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "product_categoriesedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "product_categoriesadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("product_categoriesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("product_categoriesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "product_categoriesadd.php?" . $this->UrlParm($parm);
		else
			$url = "product_categoriesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("product_categoriesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("product_categoriesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("product_categoriesdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->category_image->Upload->DbValue = $rs->fields('category_image');
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->category_image->Exportable) $Doc->ExportCaption($this->category_image);
				} else {
					if ($this->sn->Exportable) $Doc->ExportCaption($this->sn);
					if ($this->category->Exportable) $Doc->ExportCaption($this->category);
					if ($this->category_image->Exportable) $Doc->ExportCaption($this->category_image);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->category_image->Exportable) $Doc->ExportField($this->category_image);
					} else {
						if ($this->sn->Exportable) $Doc->ExportField($this->sn);
						if ($this->category->Exportable) $Doc->ExportField($this->category);
						if ($this->category_image->Exportable) $Doc->ExportField($this->category_image);
