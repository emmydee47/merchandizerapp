...
$messages_view = NULL; // Initialize page object first

class cmessages_view extends cmessages {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["sn"] <> "") {
				...
				$sReturnUrl = "messageslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "messageslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "messageslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($messages_view)) $messages_view = new cmessages_view();

// Page init
$messages_view->Page_Init();

// Page main
$messages_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$messages_view->Page_Render();
?>
...
var CurrentForm = fmessagesview = new ew_Form("fmessagesview", "view");

// Form_CustomValidate event
fmessagesview.Form_CustomValidate = 
...
fmessagesview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmessagesview.Lists["x_sender"] = {"LinkField":"x_sn","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"user"};
fmessagesview.Lists["x_sender"].Data = "<?php echo $messages_view->sender->LookupFilterQuery(FALSE, "view") ?>";
fmessagesview.Lists["x_receiver"] = {"LinkField":"x_sn","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"user"};
fmessagesview.Lists["x_receiver"].Data = "<?php echo $messages_view->receiver->LookupFilterQuery(FALSE, "view") ?>";
...
<div class="ewToolbar">
<?php $messages_view->ExportOptions->Render("body") ?>
<?php
	foreach ($messages_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $messages_view->ShowPageHeader(); ?>
<?php
$messages_view->ShowMessage();
?>
<form name="fmessagesview" id="fmessagesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($messages_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $messages_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="messages">
<input type="hidden" name="modal" value="<?php echo intval($messages_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($messages->sn->Visible) { // sn ?>
	<tr id="r_sn">
		<td class="col-sm-2"><span id="elh_messages_sn"><?php echo $messages->sn->FldCaption() ?></span></td>
		<td data-name="sn"<?php echo $messages->sn->CellAttributes() ?>>
<span id="el_messages_sn">
<span<?php echo $messages->sn->ViewAttributes() ?>>
<?php echo $messages->sn->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($messages->message_title->Visible) { // message_title ?>
	<tr id="r_message_title">
		<td class="col-sm-2"><span id="elh_messages_message_title"><?php echo $messages->message_title->FldCaption() ?></span></td>
...
<?php echo $messages->message_title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($messages->message_body->Visible) { // message_body ?>
	<tr id="r_message_body">
		<td class="col-sm-2"><span id="elh_messages_message_body"><?php echo $messages->message_body->FldCaption() ?></span></td>
		<td data-name="message_body"<?php echo $messages->message_body->CellAttributes() ?>>
<span id="el_messages_message_body">
<span<?php echo $messages->message_body->ViewAttributes() ?>>
<?php echo $messages->message_body->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($messages->sender->Visible) { // sender ?>
	<tr id="r_sender">
		<td class="col-sm-2"><span id="elh_messages_sender"><?php echo $messages->sender->FldCaption() ?></span></td>
		<td data-name="sender"<?php echo $messages->sender->CellAttributes() ?>>
<span id="el_messages_sender">
<span<?php echo $messages->sender->ViewAttributes() ?>>
<?php echo $messages->sender->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($messages->receiver->Visible) { // receiver ?>
	<tr id="r_receiver">
		<td class="col-sm-2"><span id="elh_messages_receiver"><?php echo $messages->receiver->FldCaption() ?></span></td>
...
<?php echo $messages->receiver->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($messages->message_date->Visible) { // message_date ?>
	<tr id="r_message_date">
		<td class="col-sm-2"><span id="elh_messages_message_date"><?php echo $messages->message_date->FldCaption() ?></span></td>
		<td data-name="message_date"<?php echo $messages->message_date->CellAttributes() ?>>
<span id="el_messages_message_date">
<span<?php echo $messages->message_date->ViewAttributes() ?>>
<?php echo $messages->message_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
