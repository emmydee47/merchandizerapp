...
class ccontent extends cTable {
	...
	function __construct() {
		...
		// sn
		$this->sn = new cField('content', 'content', 'x_sn', 'sn', '`sn`', '`sn`', 3, -1, FALSE, '`sn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sn->Sortable = TRUE; // Allow sort
		$this->sn->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sn'] = &$this->sn;

		// content_title
		$this->content_title = new cField('content', 'content', 'x_content_title', 'content_title', '`content_title`', '`content_title`', 200, -1, FALSE, '`content_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->content_title->Sortable = TRUE; // Allow sort
		$this->fields['content_title'] = &$this->content_title;

		// description
		$this->description = new cField('content', 'content', 'x_description', 'description', '`description`', '`description`', 200, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// file
		$this->file = new cField('content', 'content', 'x_file', 'file', '`file`', '`file`', 200, -1, TRUE, '`file`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`content`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "contentedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "contentadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("contentview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("contentview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "contentadd.php?" . $this->UrlParm($parm);
		else
			$url = "contentadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("contentedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("contentadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->content_title->setDbValue($rs->fields('content_title'));
	...
	function RenderEditRow() {
		...
		$this->content_title->EditValue = $this->content_title->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->content_title->Exportable) $Doc->ExportCaption($this->content_title);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->content_title->Exportable) $Doc->ExportField($this->content_title);
