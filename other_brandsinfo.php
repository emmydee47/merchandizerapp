...
//
// Table class for other_brands
//
class cother_brands extends cTable {
	var $sn;
	var $brand_name;
	...
	function __construct() {
		...
		$this->TableVar = 'other_brands';
		$this->TableName = 'other_brands';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`other_brands`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sn
		$this->sn = new cField('other_brands', 'other_brands', 'x_sn', 'sn', '`sn`', '`sn`', 3, -1, FALSE, '`sn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sn->Sortable = TRUE; // Allow sort
		$this->sn->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sn'] = &$this->sn;

		// brand_name
		$this->brand_name = new cField('other_brands', 'other_brands', 'x_brand_name', 'brand_name', '`brand_name`', '`brand_name`', 200, -1, FALSE, '`brand_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`other_brands`";
	}
	...
	function GetModalCaption($pageName) {
		global $Language;
		if ($pageName == "other_brandsview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "other_brandsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "other_brandsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("other_brandsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("other_brandsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "other_brandsadd.php?" . $this->UrlParm($parm);
		else
			$url = "other_brandsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("other_brandsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("other_brandsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("other_brandsdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->brand_name->setDbValue($rs->fields('brand_name'));
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->brand_name->Exportable) $Doc->ExportCaption($this->brand_name);
				} else {
					if ($this->sn->Exportable) $Doc->ExportCaption($this->sn);
					if ($this->brand_name->Exportable) $Doc->ExportCaption($this->brand_name);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->brand_name->Exportable) $Doc->ExportField($this->brand_name);
					} else {
						if ($this->sn->Exportable) $Doc->ExportField($this->sn);
						if ($this->brand_name->Exportable) $Doc->ExportField($this->brand_name);
					}
