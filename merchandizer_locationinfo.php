...
$merchandizer_location = NULL;

//
// Table class for merchandizer_location
//
class cmerchandizer_location extends cTable {
	var $sn;
	var $user_id;
	var $longitude;
	var $latitude;
	...
	function __construct() {
		...
		$this->TableVar = 'merchandizer_location';
		$this->TableName = 'merchandizer_location';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`merchandizer_location`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sn
		$this->sn = new cField('merchandizer_location', 'merchandizer_location', 'x_sn', 'sn', '`sn`', '`sn`', 3, -1, FALSE, '`sn`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sn->Sortable = TRUE; // Allow sort
		$this->sn->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sn'] = &$this->sn;

		// user_id
		$this->user_id = new cField('merchandizer_location', 'merchandizer_location', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_id->Sortable = TRUE; // Allow sort
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// longitude
		$this->longitude = new cField('merchandizer_location', 'merchandizer_location', 'x_longitude', 'longitude', '`longitude`', '`longitude`', 200, -1, FALSE, '`longitude`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->longitude->Sortable = TRUE; // Allow sort
		$this->fields['longitude'] = &$this->longitude;

		// latitude
		$this->latitude = new cField('merchandizer_location', 'merchandizer_location', 'x_latitude', 'latitude', '`latitude`', '`latitude`', 200, -1, FALSE, '`latitude`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->latitude->Sortable = TRUE; // Allow sort
		$this->fields['latitude'] = &$this->latitude;

		// location
		$this->location = new cField('merchandizer_location', 'merchandizer_location', 'x_location', 'location', '`location`', '`location`', 200, -1, FALSE, '`location`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->location->Sortable = TRUE; // Allow sort
		$this->fields['location'] = &$this->location;

		// date
		$this->date = new cField('merchandizer_location', 'merchandizer_location', 'x_date', 'date', '`date`', ew_CastDateFieldForLike('`date`', 0, "DB"), 135, 0, FALSE, '`date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`merchandizer_location`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "merchandizer_locationlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "merchandizer_locationedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "merchandizer_locationadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("merchandizer_locationview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("merchandizer_locationview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "merchandizer_locationadd.php?" . $this->UrlParm($parm);
		else
			$url = "merchandizer_locationadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("merchandizer_locationedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("merchandizer_locationadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("merchandizer_locationdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->longitude->Exportable) $Doc->ExportCaption($this->longitude);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->longitude->Exportable) $Doc->ExportField($this->longitude);
