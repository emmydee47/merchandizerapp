...
function CurrentProjectID() {
	...
	return "{B6A7A568-E606-4B9C-A82F-C80372444DA0}";
}
...
class cBreadcrumb {
	...
	function __construct() {
		global $Language;
		$this->Links[] = array("home", "HomePage", "transactionslist.php", "ewHome", "", FALSE); // Home
	}
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($valid) {
						...
						$this->setSessionUserID($rs->fields('sn')); // Load User ID
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('sn'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
